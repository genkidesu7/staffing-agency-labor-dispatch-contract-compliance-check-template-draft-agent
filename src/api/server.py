...
app = FastAPI(title="SVC-C2-047 Staffing Contract Compliance Agent")

agent = SvcC2047StaffingContractComplianceAgent()
agent.compile()
agent.provision_secrets(secrets_factory(namespace="svc", agent_name="svc-c2-047"))
...
async def invoke(req: InvokeRequest, request: Request) -> dict[str, Any]:
    ...
    with bound_secrets(agent._secrets_provider):
        ...
        # agent.invoke() is sync; offload it so it doesn't block uvicorn's
        # event loop. asyncio.to_thread propagates the current contextvars
        # Context, which bound_secrets() above relies on (implementation_rule.md §3).
        return await asyncio.to_thread(agent.invoke, req.input, ctx=ctx)


@app.get("/health")
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "svc-c2-047"}
